...
class VariationalBayes(object):
    ...
    def _initialize(self, data, truncation_level=5, alpha=1., sigma_a=1., sigma_x=1.):
        ...
        if self._finite_mode:
            ...
            self._tau += 0.5 * min(1., self._alpha / self._K) * (numpy.random.random(self._tau.shape) - 0.5)
        else:
            self._tau[0, :] = self._alpha
            self._tau += 0.5 * min(1., self._alpha) * (numpy.random.random(self._tau.shape) - 0.5)
