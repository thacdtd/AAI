...
class PoissonMF(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X):
        ...
        :param X: array-like, shape (n_samples, n_feats)
            Training data.
        :return: self: object
    ...
    def _update(self, X, update_beta=True):
        ...
        for i in xrange(self.max_iter):
            ...
            if self.verbose:
                sys.stdout.write('\r\tAfter ITERATION: %d\tObjective: %.2f\t Old objective: %.2f\t'
...
class OnlinePoissonMF(PoissonMF):
    ...
    def __init__(self, n_components=100, batch_size=10, n_pass=10, max_iter=100, tol=0.0005, shuffle=True,
        ...
        :param tol: float
            The threshold on the increase of the objective to stop theiteration
        :param shuffle: bool
    ...
    def partial_fit(self, X):
        ...
        ratio = X / self._xexplog()
        self.gamma_b = (1 - self.rho) * self.gamma_b + self.rho * (self.b + self._scale * np.exp(self.Elogb) *
                                                                   np.dot(np.exp(self.Elogt).T, ratio))
        self.rho_b = (1 - self.rho) * self.rho_b + self.rho * (self.b + self._scale *
                                                               np.sum(self.Et, axis=0, keepdims=True).T)
        self.Eb, self.Elogb = _compute_expectations(self.gamma_b, self.rho_b)
        return self
